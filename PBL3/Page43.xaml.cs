        ...
        private async Task LoadDataAsync()
        {
            try
            {
                ...
                if (violation != null)
                {
                    ...
                    txtNgayViPham.Text = violation.ViolationDate?.ToString("dd/MM/yyyy") ?? "Chưa xác đ?nh";
                    txtGioViPham.Text = violation.ViolationTime?.ToString(@"hh\:mm") ?? "Chưa xác đ?nh";
                    txtDiaDiemViPham.Text = violation.Address ?? "Chưa r?";
                    txtMoTaViPham.Text = violation.ViolationDescription ?? "Chưa có chi ti?t";

                    // We can format the fines/points further if needed based on the table
                    txtMucPhat.Text = "Chưa c?p nh?t";
                    txtTruDiem.Text = violation.DemeritPoints ?? "0 Đi?m";
                    txtDiaDiemDongPhat.Text = "Chưa c?p nh?t";

                    if (violation.Status == 0) {
                        txtTrangThai.Text = "Chưa x? l?";
