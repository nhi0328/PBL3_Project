        ...
        private async Task LoadDataAsync()
        {
            ...
            try
            {
                ...
                if (vehicle != null)
                {
                    ...
                    txtLoaiXe.Text = vehicle.VehicleType?.Category?.CategoryName ?? "Chưa xác đ?nh";
                    txtNhanHieu.Text = vehicle.VehicleType?.VehicleTypeName ?? "Chưa xác đ?nh";
                    txtNamSanXuat.Text = vehicle.VehicleType?.ManufactureYear?.ToString() ?? "Chưa xác đ?nh";
                    txtMauSac.Text = vehicle.VehicleType?.Color?.ColorName ?? "Chưa xác đ?nh";

                    txtChuSoHuu.Text = vehicle.Owner?.FullName ?? "Chưa xác đ?nh";
                    txtSoKhung.Text = vehicle.ShassisNumber ?? "Chưa xác đ?nh";
                    txtSoMay.Text = vehicle.EngineNumber ?? "Chưa xác đ?nh";
                    txtNgayDangKy.Text = vehicle.RegistrationDate?.ToString("dd/MM/yyyy") ?? "Chưa xác đ?nh";
                ...
                foreach (var v in violations)
                {
                    ...
                    violationItems.Add(new ViolationItem
                    {
                        ...
                        Location = v.Address ?? "Chưa r?",
                        Status = v.Status == 0 ? "Chưa x? l?" : "Đ? x? l?"
                    });
                }
            ...
            {
                new CustomMessageBox("L?i t?i chi ti?t Phương ti?n: " + ex.Message, "L?i k?t n?i").ShowDialog();
            }
        }
